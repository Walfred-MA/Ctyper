...
public:
    ...
    void resize(const vector<uint>& sizes)
    {
        ...
        {
            windowcovers[i].resize(sizes[i]/window + 1, make_tuple(0,0,0));
        }
    }
    
    void WindowCovers(const uint16* kmervec, const uint16* kmermatrix, const FLOAT_T depth, const uint16 gnum, const uint knum, const int genenum, const int* results);
